...
class EvidenceCourtSmokeScriptTest(unittest.TestCase):
    ...
    def test_technical_trend_radar_keeps_external_project_boundaries(self) -> None:
        ...
        self.assertIn("https://github.com/anomalyco/opencode", text)
        self.assertIn("https://opencode.ai/docs/", text)
        self.assertIn("## Fresh Source Receipts", text)
        self.assertIn("Public docs emphasize sandboxed exec, approvals, allowlists", text)
        ...
        self.assertIn("## Star-Readiness Lessons", text)
        self.assertIn("Every sent/replied/shared state needs a public URL", text)
        ...
        self.assertIn("Do not\nscale trend-watch outreach until the issue #5 re-check has a public posted URL", text)
    ...
    def test_release_set_script_lists_and_checks_staging_boundary(self) -> None:
        ...
        self.assertEqual(usage_proc.returncode, 0, usage_proc.stdout + usage_proc.stderr)
        self.assertIn("--render-current-main-review-packet", usage_proc.stdout)
    ...
    def test_release_set_script_renders_current_main_review_packet_without_placeholders(self) -> None:
        root = Path(__file__).resolve().parents[1]
        script = root / "scripts" / "evidence_court_release_set.sh"
        env = os.environ.copy()
        env["EVIDENCE_COURT_MAIN_SHA"] = "0123456789abcdef0123456789abcdef01234567"
        env["EVIDENCE_COURT_RUN_URL"] = (
            "https://github.com/1966536805l-crypto/openmako-evidence-court/actions/runs/123456789"
        )

        proc = subprocess.run(
            ["bash", str(script), "--render-current-main-review-packet"],
            cwd=root,
            env=env,
            text=True,
            capture_output=True,
        ...
        self.assertIn("Main commit: 0123456789abcdef0123456789abcdef01234567", proc.stdout)
        self.assertIn(
            "CI: https://github.com/1966536805l-crypto/openmako-evidence-court/actions/runs/123456789",
            proc.stdout,
        )
        self.assertIn("Artifact digest: not included here", proc.stdout)
        self.assertIn("agent_runtime", proc.stdout)
        self.assertIn("tool_calls", proc.stdout)
        self.assertIn("approval_events", proc.stdout)
        self.assertIn("sandbox_boundary", proc.stdout)
        self.assertIn("reviewer context only", proc.stdout)
        self.assertIn("They do not prove", proc.stdout)
        self.assertIn("technical boundary check", proc.stdout)
        self.assertIn("Not asking for endorsement, adoption, or a share.", proc.stdout)
        self.assertNotIn("<MAIN_SHA>", proc.stdout)
        self.assertNotIn("<RUN_URL>", proc.stdout)
        self.assertNotIn("<artifact_digest>", proc.stdout.lower())
    ...
    def test_readme_separates_other_modules_from_v0_1_launch_claim(self) -> None:
        ...
        self.assertIn("## Integration-Shaped Examples", readme)
        self.assertIn("examples/evidence-court/terminal-agent-bad-run.json", readme)
        self.assertIn("examples/evidence-court/local-gateway-bad-run.json", readme)
        self.assertIn("--fail-on-reason-code test.required_not_run --json", readme)
        self.assertIn("not native ingestion", readme)
        self.assertIn("reviewer context only", readme)
        self.assertIn("do not prove real\napproval, sandbox enforcement, or external integration", readme)
        ...
        self.assertNotIn("native Hermes ingestion", readme)
        self.assertNotIn("native OpenClaw ingestion", readme)
    ...
    def test_current_proof_status_and_review_issue_draft_do_not_overclaim_remote_evidence(self) -> None:
        ...
        self.assertIn("actions/runs/26967812515", proof_status)
        self.assertIn("completed successfully", proof_status)
        self.assertIn("sha256:b753e0563d63ca5c0f9c1850b67a13ec9fb2f8f53e5591b9260a93d77561332e", proof_status)
        self.assertIn("reason-codes.json", proof_status)
        self.assertIn("reason-codes.md", proof_status)
        self.assertIn("does not automatically prove current or later main commits", proof_status)
    ...
    def test_launch_assets_keep_v0_1_claims_bounded(self) -> None:
        ...
        self.assertIn("reason-codes.md", launch_post)
        self.assertIn("test.required_not_run", launch_post)
    ...
    def test_expert_review_brief_keeps_share_claims_verifiable(self) -> None:
        ...
        self.assertIn("reason-codes.json", brief)
        self.assertIn("reason-codes.md", brief)
        self.assertIn("test.required_not_run", brief)
        self.assertIn("expert review brief", readme)
    ...
    def test_outreach_templates_are_review_first_and_bounded(self) -> None:
        ...
        self.assertIn("five-agent AI-likeness check", outreach)
        self.assertIn("template-like, too complete, or too\nannouncement-shaped", outreach)
        self.assertIn("I'm trying to make agent-run failure reports less vague.", outreach)
        self.assertIn("submitted run record never shows the\nrequired test command", outreach)
        self.assertIn("Right now the checker reports", outreach)
        self.assertIn("Demo card", outreach)
        self.assertIn("Would you keep that reason code, rename it, or track a different field?", outreach)
        ...
        self.assertIn("not reading native\nClaude/Codex/Cursor or CI logs yet", outreach)
    ...
    def test_outreach_targets_are_candidates_and_bounded(self) -> None:
        ...
        self.assertIn("https://github.com/anomalyco/opencode", targets)
        self.assertNotIn("https://github.com/sst/opencode", targets)
    ...
    def test_current_main_evidence_packet_is_refresh_safe_and_not_a_share_request(self) -> None:
        ...
        for phrase in (
            ...
            "contains the current main SHA",
            "completed successfully",
            "cannot resolve a successful public",
            "exact current main commit",
            "Repo: https://github.com/1966536805l-crypto/openmako-evidence-court",
            "Main commit: generated by scripts/evidence_court_release_set.sh",
            "CI: generated by scripts/evidence_court_release_set.sh",

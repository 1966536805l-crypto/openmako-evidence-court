...
class EvidenceCourtSmokeScriptTest(unittest.TestCase):
    ...
    def _copy_release_source_for_smoke(self, root: Path, target: Path) -> None:
        file_paths = (
    ...
    def test_smoke_script_runs_release_gate(self) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="openmako-smoke-index-") as index_dir:
    ...
    def test_smoke_script_rejects_extra_packaged_quantagent_module(self) -> None:
        root = Path(__file__).resolve().parents[1]
        with tempfile.TemporaryDirectory(prefix="openmako-package-boundary-negative-") as tmp:
            candidate = Path(tmp) / "candidate"
            candidate.mkdir()
            self._copy_release_source_for_smoke(root, candidate)
            (candidate / "quantagent" / "accidental_runtime.py").write_text(
                "def leaked() -> str:\n    return 'runtime module must not ship'\n",
                encoding="utf-8",
            )
            setup_path = candidate / "setup.py"
            setup_path.write_text(
                setup_path.read_text(encoding="utf-8").replace(
                    'EVIDENCE_COURT_PACKAGE_MODULES = {"__init__", "evidence_court"}',
                    'EVIDENCE_COURT_PACKAGE_MODULES = {"__init__", "evidence_court", "accidental_runtime"}',
                ),
                encoding="utf-8",
            )
        ...
        self.assertIn("wheel quantagent module boundary changed", output)
        self.assertIn("accidental_runtime.py", output)
        self.assertNotIn("sdist quantagent module boundary changed", output)

    def test_smoke_script_rejects_extra_sdist_quantagent_module(self) -> None:
        root = Path(__file__).resolve().parents[1]
        with tempfile.TemporaryDirectory(prefix="openmako-sdist-boundary-negative-") as tmp:
            candidate = Path(tmp) / "candidate"
            candidate.mkdir()
            self._copy_release_source_for_smoke(root, candidate)
            (candidate / "quantagent" / "accidental_sdist.py").write_text(
                "def leaked() -> str:\n    return 'sdist module must not ship'\n",
                encoding="utf-8",
            )
            manifest_path = candidate / "MANIFEST.in"
            manifest_path.write_text(
                manifest_path.read_text(encoding="utf-8").replace(
                    "include quantagent/evidence_court.py",
                    "include quantagent/evidence_court.py\ninclude quantagent/accidental_sdist.py",
                ),
                encoding="utf-8",
            )
            git_init = subprocess.run(
        ...
        output = proc.stdout + proc.stderr
        self.assertNotEqual(proc.returncode, 0, output)
        self.assertNotIn("wheel quantagent module boundary changed", output)
        self.assertIn("sdist quantagent module boundary changed", output)
        self.assertIn("accidental_sdist.py", output)
    ...
    def test_release_set_branch_diff_default_matches_release_docs(self) -> None:
        root = Path(__file__).resolve().parents[1]
        script = root / "scripts" / "evidence_court_release_set.sh"
        text = script.read_text(encoding="utf-8")

        self.assertIn("BASE_REF defaults to\norigin/main.", text)
        self.assertIn('${EVIDENCE_COURT_BASE_REF:-origin/main}', text)
        self.assertNotIn('${EVIDENCE_COURT_BASE_REF:-main}', text)

    def test_release_docs_markdown_links_resolve(self) -> None:
        root = Path(__file__).resolve().parents[1]
        release_docs = (
            root / "README.md",
            root / "docs" / "CAPABILITY_GATES.md",
            root / "docs" / "EVIDENCE_COURT_V0_1_LAUNCH_PACKET.md",
            root / "docs" / "EVIDENCE_COURT_V0_1_PACKAGE_README.md",
            root / "docs" / "EVIDENCE_COURT_V0_1_PR_BODY.md",
            root / "docs" / "EVIDENCE_COURT_V0_1_RELEASE_CUT.md",
            root / "docs" / "EVIDENCE_COURT_V0_1_RELEASE_MANIFEST.md",
        )
    ...
    def test_smoke_script_writes_review_artifacts_when_requested(self) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="openmako-smoke-artifacts-") as artifacts:
            ...
            self.assertEqual('"verdict": "PASS"', manifest["expected_checks"]["github-actions-test-step.json"])
            self.assertEqual('"verdict": "PASS"', manifest["expected_checks"]["github-actions-job-log.json"])
            self.assertEqual('"verdict": "FAIL"', manifest["expected_checks"]["failed-pytest-log.json"])
            ...
            self.assertEqual(
                "github-actions-test-step.log",
                manifest["source_provenance_checks"]["github-actions-test-step.json"],
            )
            self.assertEqual(
                "github-actions-job-log.log",
                manifest["source_provenance_checks"]["github-actions-job-log.json"],
            )
            self.assertEqual(
                "failed-pytest-log.log",
                manifest["source_provenance_checks"]["failed-pytest-log.json"],
            )
            ...
            self.assertIn("No native Claude/Codex/Cursor/Devin transcript ingestion claim.", manifest["boundaries"])
            self.assertIn(
            ...
            self.assertIn("No proof that tests ran outside the supplied record or supplied log.", manifest["boundaries"])
            ...
            self.assertIn("OpenMako AgentRunResult-shaped producer record returns", quickstart)
            self.assertIn("verification_required` but no test command", quickstart)
            self.assertIn("Open `github-actions-test-step.log`, then `github-actions-test-step.json`", quickstart)
            self.assertIn("supplied GitHub Actions pytest test-step log", quickstart)
            self.assertIn("Open `github-actions-job-log.log`, then `github-actions-job-log.json`", quickstart)
            self.assertIn("supplied full GitHub Actions job log", quickstart)
            self.assertIn("Open `failed-pytest-log.log`, then `failed-pytest-log.json`", quickstart)
            self.assertIn("supplied failed pytest log returns", quickstart)
            ...
            self.assertIn("OpenMako AgentRunResult-shaped input fails closed", quickstart)
            self.assertIn("supplied GitHub Actions pytest test-step log input passes", quickstart)
            self.assertIn("supplied full GitHub Actions job log input passes", quickstart)
            self.assertIn("supplied failed pytest log input fails closed", quickstart)
            ...
            self.assertIn("supplied pytest/GitHub Actions test-step logs", quickstart)
            self.assertIn("supplied GitHub Actions job logs", quickstart)
            self.assertIn("does not prove native Claude/Codex/Cursor/Devin transcript ingestion", quickstart)
            self.assertIn("arbitrary CI ingestion", quickstart)
            self.assertIn("tests really ran outside the supplied record/log", quickstart)
            ...
            github_actions_json = (artifact_root / "github-actions-test-step.json").read_text(encoding="utf-8")
            github_actions_job_json = (artifact_root / "github-actions-job-log.json").read_text(encoding="utf-8")
            failed_pytest_json = (artifact_root / "failed-pytest-log.json").read_text(encoding="utf-8")
            for report_json in (
                fail_on_json,
                good_json,
                marked_json,
            ...
            self.assertIn("source: openmako-agent-result-shaped-record", openmako_json)
            self.assertIn("required test not run: validate", openmako_json)
            self.assertIn("required test not run: unit_tests", openmako_json)
            github_actions_log = (artifact_root / "github-actions-test-step.log").read_text(encoding="utf-8")
            failed_pytest_log = (artifact_root / "failed-pytest-log.log").read_text(encoding="utf-8")
            self.assertIn("##[group]Run python -m pytest tests/test_calculator.py -q", github_actions_log)
            self.assertIn("1 passed in 0.02s", github_actions_log)
            self.assertIn('"verdict": "PASS"', github_actions_json)
            self.assertIn("github-actions-test-step.log", github_actions_json)
            self.assertIn("test command observed: python -m pytest tests/test_calculator.py -q", github_actions_json)
            github_actions_job_log = (artifact_root / "github-actions-job-log.log").read_text(encoding="utf-8")
            self.assertIn("##[group]Run actions/setup-python@v5", github_actions_job_log)
            self.assertIn("##[group]Run python -m pytest tests/test_evidence_court.py", github_actions_job_log)
            self.assertIn("69 passed in 18.47s", github_actions_job_log)
            self.assertIn('"verdict": "PASS"', github_actions_job_json)
            self.assertIn("github-actions-job-log.log", github_actions_job_json)
            self.assertIn(
                "test command observed: python -m pytest tests/test_evidence_court.py tests/test_evidence_court_smoke_script.py -q",
                github_actions_job_json,
            )
            self.assertIn("$ python -m pytest tests/test_calculator.py -q", failed_pytest_log)
            self.assertIn("1 failed in 0.03s", failed_pytest_log)
            self.assertIn('"verdict": "FAIL"', failed_pytest_json)
            self.assertIn("failed-pytest-log.log", failed_pytest_json)
            self.assertIn("test output status: failed", failed_pytest_json)
            ...
            self.assertIn("OpenMako AgentRunResult-shaped record gate checked verification_required", summary)
            ...
            self.assertIn(
                "CI-log gate checked a supplied GitHub Actions pytest test-step log returns PASS",
                summary,
            )
            self.assertIn("a supplied full GitHub Actions job log returns PASS", summary)
            ...
            self.assertIn('github-actions-test-step.json must contain "verdict": "PASS"', summary)
            self.assertIn('github-actions-job-log.json must contain "verdict": "PASS"', summary)
            self.assertIn('failed-pytest-log.json must contain "verdict": "FAIL"', summary)
    ...
    def test_github_actions_workflow_runs_smoke_script(self) -> None:
        ...
        self.assertIn("python -m pip install --upgrade pip pytest wheel", text)
        self.assertIn("EVIDENCE_COURT_BRANCH_DIFF_BASE:", text)
        ...
        self.assertIn('EVIDENCE_COURT_DIRECT_CONSOLE_SMOKE: "1"', text)
        ...
        self.assertRegex(
            text,
            re.compile(
                r"- name: Run Evidence Court smoke gate\s+env:\s+EVIDENCE_COURT_BRANCH_DIFF_BASE:[\s\S]*github\.event_name == 'pull_request'[\s\S]*github\.event\.before[\s\S]*EVIDENCE_COURT_ARTIFACT_DIR: evidence-court-artifacts[\s\S]*EVIDENCE_COURT_DIRECT_CONSOLE_SMOKE: \"1\"\s+run: bash scripts/evidence_court_smoke\.sh",
                re.MULTILINE,
            ),
        )
        self.assertLess(
    ...
    def test_packaging_metadata_declares_examples_and_project_urls(self) -> None:
        root = Path(__file__).resolve().parents[1]
        setup_text = (root / "setup.py").read_text(encoding="utf-8")
        pyproject_text = (root / "pyproject.toml").read_text(encoding="utf-8")
        manifest_text = (root / "MANIFEST.in").read_text(encoding="utf-8")
        smoke_text = (root / "scripts" / "evidence_court_smoke.sh").read_text(encoding="utf-8")
        package_readme_text = (
            root / "docs" / "EVIDENCE_COURT_V0_1_PACKAGE_README.md"
        ...
        pr_body_text = (root / "docs" / "EVIDENCE_COURT_V0_1_PR_BODY.md").read_text(
            encoding="utf-8"
        )

        for text in (setup_text, pyproject_text):
            self.assertIn("https://github.com/1966536805l-crypto/openmako-evidence-court", text)
            self.assertIn("OpenMako contributors", text)
            self.assertIn("1966536805l-crypto@users.noreply.github.com", text)
            self.assertIn("MIT", text)
            self.assertIn("0.1.3", text)
            self.assertIn("docs/EVIDENCE_COURT_V0_1_PACKAGE_README.md", text)
            self.assertIn("quantagent.evidence_court:main", text)
            self.assertNotIn("quantagent.cli:main", text)
            self.assertIn("examples/evidence-court/bad-run.json", text)
            self.assertIn("examples/evidence-court/good-run.json", text)
            self.assertIn("share/open-mako/examples/evidence-court", text)

        self.assertTrue((root / "LICENSE").is_file())
        self.assertIn('packages=["quantagent"]', setup_text)
        self.assertIn('EVIDENCE_COURT_PACKAGE_MODULES = {"__init__", "evidence_court"}', setup_text)
        self.assertIn('include = ["quantagent"]', pyproject_text)
        self.assertIn('readme = "docs/EVIDENCE_COURT_V0_1_PACKAGE_README.md"', pyproject_text)
        self.assertIn("license-files = [\"LICENSE\"]", pyproject_text)
        self.assertIn("exclude README.md", manifest_text)
        self.assertIn("include LICENSE", manifest_text)
        self.assertIn("include docs/EVIDENCE_COURT_V0_1_PACKAGE_README.md", manifest_text)
        self.assertIn("recursive-exclude quantagent *", manifest_text)
        self.assertIn("include quantagent/__init__.py", manifest_text)
        self.assertIn("include quantagent/evidence_court.py", manifest_text)
        self.assertIn("include examples/evidence-court/bad-run.json", manifest_text)
        self.assertIn("include examples/evidence-court/good-run.json", manifest_text)
        self.assertIn('cp -R quantagent "${src_dir}/"', smoke_text)
        self.assertNotIn('cp quantagent/__init__.py quantagent/evidence_court.py "${src_dir}/quantagent/"', smoke_text)
        self.assertIn("wheel quantagent module boundary changed", smoke_text)
        self.assertIn("sdist quantagent module boundary changed", smoke_text)
        self.assertIn("wheel missing LICENSE file", smoke_text)
        self.assertIn("wheel metadata missing MIT license", smoke_text)
        self.assertIn("wheel metadata missing License-File", smoke_text)
        self.assertIn("sdist missing LICENSE", smoke_text)
        self.assertIn("sdist missing package README", smoke_text)
        for text in (setup_text, pyproject_text, manifest_text):
            self.assertNotIn("vendor/hermes", text)
            self.assertNotIn("upstream_refs", text)
            self.assertNotIn("THIRD_PARTY_NOTICES", text)
        self.assertIn("OpenMako Evidence Court", package_readme_text)
        self.assertIn("This package installs only the narrow package surface", package_readme_text)
        self.assertIn("supplied pytest logs, GitHub Actions test-step logs, or GitHub Actions job", package_readme_text)
        self.assertIn("does not natively ingest Claude Code, Codex, Cursor, Devin", package_readme_text)
        self.assertIn("arbitrary CI logs", package_readme_text)
        self.assertIn("--from-ci-log pytest.log", package_readme_text)
        self.assertIn(
            "does not prove tests really ran outside the supplied record",
            " ".join(package_readme_text.split()),
        )
        for forbidden in (
            "Extreme Planner",
            "mako quant",
            ...
            self.assertNotIn(forbidden, package_readme_text)
        boundary = "The open-mako wheel for this release installs only the narrow quantagent package surface required by Evidence Court"
        for text in (readme_text, release_manifest_text, pr_body_text):
            normalized = " ".join(text.split())
            self.assertIn(boundary, normalized)
            self.assertIn(
                "Broader OpenMako and quantagent runtime modules are outside this launch claim unless covered by separate gates.",
                normalized,
            )
    ...
    def test_release_cut_checklist_separates_evidence_court_from_planner_work(self) -> None:
        ...
        self.assertIn("After the release commit and before remote publication:", text)
        self.assertIn("bash scripts/evidence_court_release_set.sh --check-branch-diff origin/main", text)
    ...
    def test_release_manifest_is_the_claim_file_boundary(self) -> None:
        ...
        self.assertIn("supplied pytest/GitHub Actions test-step logs", text)
        self.assertIn("supplied GitHub Actions job logs", text)
        self.assertIn("arbitrary CI log ingestion", text)
        self.assertIn("beyond supplied pytest logs", text)
    ...
    def test_launch_packet_keeps_public_claims_bounded(self) -> None:
        ...
        self.assertIn("Supplied pytest/GitHub Actions logs are audited only when", text)
        self.assertIn("Arbitrary CI logs are ingested.", text)
        self.assertIn("inside the supplied", text)
        self.assertIn("supplied pytest/GitHub Actions", text)
        ...
        self.assertIn("reviewer-quickstart.md", text)
        self.assertIn("smoke-summary.txt", text)
        self.assertIn("github-actions-test-step.json", text)
        self.assertIn("github-actions-job-log.json", text)
        self.assertIn("failed-pytest-log.json", text)
        self.assertIn("Artifact content check", text)
        self.assertIn("open `bad-run.md` first", text)
        self.assertIn("source provenance checks", text)
        self.assertIn("report artifacts include `source: ...`", text)
        ...
        self.assertIn("`github-actions-test-step.json` shows `\"verdict\": \"PASS\"`", text)
        self.assertIn("`github-actions-job-log.json` shows `\"verdict\": \"PASS\"`", text)
        self.assertIn("`failed-pytest-log.json` shows `\"verdict\": \"FAIL\"`", text)
        ...
        self.assertIn("`quantagent/cli.py`", text)
    ...
    def test_capability_gates_use_bash_smoke_invocation(self) -> None:
        ...
        self.assertIn("bash scripts/evidence_court_smoke.sh", text)
        self.assertIn("Supplied pytest/GitHub Actions log auditing", text)
        self.assertIn("v0.1 narrow support, not native CI ingestion", text)
        self.assertIn(
            "smoke artifacts `github-actions-test-step.json`, `github-actions-job-log.json`, and `failed-pytest-log.json`",
            text,
        )
        self.assertIn("github-actions-job-log.json", text)
        self.assertIn("arbitrary CI logs", text)
        self.assertNotIn("LAUNCH_PLAYBOOK.md", text)
        self.assertIn("EVIDENCE_COURT_V0_1_LAUNCH_PACKET.md", text)
        self.assertTrue((root / "docs" / "EVIDENCE_COURT_V0_1_LAUNCH_PACKET.md").is_file())
        self.assertIn(
        ...
        self.assertNotIn("not part of the v0.1 public claim", text)
    ...
    def test_pr_body_is_copyable_and_keeps_claims_bounded(self) -> None:
        ...
        self.assertIn("inside the supplied", normalized)
        self.assertIn("supplied record or supplied log", normalized)
        self.assertIn("mako evidence-court --demo bad-run", text)
        self.assertIn("bash scripts/evidence_court_smoke.sh", text)
        self.assertIn("bash scripts/evidence_court_smoke.sh --artifact-dir /tmp/evidence-court-smoke", text)
        ...
        self.assertIn("`github-actions-test-step.json`: supplied GitHub Actions pytest test-step log", text)
        self.assertIn("`github-actions-job-log.json`: supplied GitHub Actions job log", text)
        self.assertIn("`failed-pytest-log.json`: supplied failed pytest log", text)
        ...
        self.assertIn("reviewer-quickstart.md", text)
        self.assertIn("smoke-summary.txt", text)
        self.assertIn("github-actions-test-step.log", text)
        self.assertIn("github-actions-test-step.json", text)
        self.assertIn("github-actions-job-log.log", text)
        self.assertIn("github-actions-job-log.json", text)
        self.assertIn("failed-pytest-log.log", text)
        self.assertIn("failed-pytest-log.json", text)
        self.assertIn("Artifact content check", text)
        ...
        self.assertIn("`bad-run.md` shows `Verdict: FAIL`", text)
        self.assertIn("`good-run.json` shows `\"verdict\": \"PASS\"`", text)
        self.assertIn("`github-actions-test-step.json` shows", text)
        self.assertIn("`github-actions-job-log.json` shows", text)
        self.assertIn("`failed-pytest-log.json` shows", text)
        self.assertIn("`mixed-source-rejection.txt` shows", text)
        self.assertIn("Optional remote artifact download", text)
        self.assertIn("### Local Artifact Review Path", text)
        self.assertIn("Before remote CI exists:", text)
        ...
        forbidden_positive_claims = (
            "native Claude/Codex/Cursor/Devin transcript ingestion is supported",
            "full CI job-log ingestion is supported",
            "Evidence Court proves tests really ran outside the supplied record",
            "OpenMako proves broad SWE-style repository repair",
            "OpenMako has achieved Desktop L4/L5 autonomy",
        )
    ...
    def test_release_set_script_lists_and_checks_staging_boundary(self) -> None:
        ...
        env = os.environ.copy()
        with tempfile.TemporaryDirectory(prefix="openmako-release-list-index-") as tmp:
            env["GIT_INDEX_FILE"] = str(Path(tmp) / "index")
            read_tree = subprocess.run(
                ["git", "read-tree", "HEAD"],
                cwd=root,
    ...
    def test_release_set_script_requires_complete_staged_release_set_when_requested(self) -> None:
        ...
        self.assertIn(
            "missing staged release file for Evidence Court v0.1:",
            empty.stdout + empty.stderr,
        )
        self.assertNotIn(
            "Staged files plus existing release files cover the Evidence Court v0.1 release set.",
            empty.stdout,
        )
        ...
        script = root / "scripts" / "evidence_court_release_set.sh"
        env = os.environ.copy()
        with tempfile.TemporaryDirectory(prefix="openmako-release-index-existing-") as tmp:
            env["GIT_INDEX_FILE"] = str(Path(tmp) / "index")
            read_tree = subprocess.run(
                ["git", "read-tree", "HEAD"],
                cwd=root,
            ...
            existing_blob = subprocess.run(
                ["git", "hash-object", "-w", "--stdin"],
                cwd=root,
                env=env,
                input=(root / "LICENSE").read_text(encoding="utf-8"),
                text=True,
                capture_output=True,
                timeout=30,
            )
            self.assertEqual(existing_blob.returncode, 0, existing_blob.stdout + existing_blob.stderr)
            replace_existing = subprocess.run(
                [
                    "git",
                    "update-index",
                    "--cacheinfo",
                    "100644",
                    existing_blob.stdout.strip(),
                    "LICENSE",
                ],
                cwd=root,
                env=env,
            ...
            self.assertEqual(replace_existing.returncode, 0, replace_existing.stdout + replace_existing.stderr)
            existing_in_base = subprocess.run(
                ["bash", str(script), "--check-staged-release-set"],
                cwd=root,
                env=env,
                text=True,
                capture_output=True,
            ...
            self.assertEqual(existing_in_base.returncode, 0, existing_in_base.stdout + existing_in_base.stderr)
            self.assertIn(
                "Staged files plus existing release files cover the Evidence Court v0.1 release set.",
                existing_in_base.stdout,
            )
    ...
    def test_release_set_script_audits_staged_claim_copy(self) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="openmako-claim-copy-index-") as tmp:
            ...
            original_blob = subprocess.run(
                ...
                cwd=root,
                env=env,
                input=pr_body.read_text(encoding="utf-8"),
                text=True,
                capture_output=True,
                timeout=30,
            )
            self.assertEqual(original_blob.returncode, 0, original_blob.stdout + original_blob.stderr)
            ...
            weakened_readme = readme.read_text(encoding="utf-8").replace(
            ...
            readme_blob = subprocess.run(
                ["git", "hash-object", "-w", "--stdin"],
                cwd=root,
                env=env,
                input=weakened_readme,
                text=True,
                capture_output=True,
                timeout=30,
            )
            self.assertEqual(readme_blob.returncode, 0, readme_blob.stdout + readme_blob.stderr)
            weaken_readme = subprocess.run(
                [
                    "git",
                    "update-index",
                    "--cacheinfo",
                    "100644",
                    readme_blob.stdout.strip(),
                    "README.md",
                ],
                cwd=root,
                env=env,
            ...
            self.assertEqual(weaken_readme.returncode, 0, weaken_readme.stdout + weaken_readme.stderr)
            package_boundary_audit = subprocess.run(
                ["bash", str(script), "--audit-staged-claim-copy"],
                cwd=root,
                env=env,
                text=True,
            ...
            self.assertNotEqual(
                package_boundary_audit.returncode,
                0,
                package_boundary_audit.stdout + package_boundary_audit.stderr,
            )
            self.assertIn(
                "staged claim-copy audit missing in README.md: "
                "Packaging boundary: The open-mako wheel for this release installs only the",
                package_boundary_audit.stdout + package_boundary_audit.stderr,
            )
    ...
    def test_release_set_script_rejects_branch_diff_without_merge_base(self) -> None:
        root = Path(__file__).resolve().parents[1]
        source_script = root / "scripts" / "evidence_court_release_set.sh"
        release_paths = (
            ".github/workflows/evidence-court.yml",
        ...
        with tempfile.TemporaryDirectory(prefix="openmako-branch-no-merge-base-") as tmp:
            repo = Path(tmp)
            script = repo / "scripts" / "evidence_court_release_set.sh"
            for path in release_paths:
                target = repo / path
            ...
            self._git(repo, "add", "--", ".")
            self._git(repo, "commit", "-m", "baseline")
            self._git(repo, "checkout", "--orphan", "orphan-release")
            (repo / "README.md").write_text("orphan release\n", encoding="utf-8")
            self._git(repo, "add", "--", ".")
            self._git(repo, "commit", "-m", "orphan release")
            ...
            self.assertIn("no merge base for Evidence Court branch diff: main...HEAD", proc.stderr)
    ...
    def test_readme_separates_other_modules_from_v0_1_launch_claim(self) -> None:
        ...
        self.assertIn("supplied pytest/GitHub Actions", readme)
        self.assertIn("supplied GitHub Actions job logs", readme)
        self.assertIn("does not parse raw", readme)
        self.assertIn("chat transcripts", readme)
        self.assertIn("Claude/Codex/Cursor/Devin logs", readme)
        self.assertIn("arbitrary CI logs", readme)
        ...
        self.assertNotIn("<summary>Other OpenMako modules", readme)
        self.assertNotIn("Extreme Planner", readme)
        self.assertNotIn("mako quant", readme)
        self.assertNotIn("Agent Failure Autopsy", readme)
        self.assertNotIn("OpenClaw-style", readme)
        ...
        self.assertIn("The open-mako wheel for this release installs only the\nnarrow quantagent package surface", quick_start)
        self.assertIn("Broader OpenMako and\nquantagent runtime modules are outside this launch claim", quick_start)
